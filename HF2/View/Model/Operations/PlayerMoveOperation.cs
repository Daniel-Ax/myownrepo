...
namespace HF2.Model.Operations
{
    public class PlayerMoveOperation : OperationBase
    {
        ...
        public override bool TryExecute(Field currentField, Field selectedField)
        {
        ...
        private bool UserCanHit(int oldXCoor, int oldYCoor, int newXCoor, int newYCoor)
        {
            ...
            for (int row = oldXCoor - 1; row > newXCoor; row--)
            {
                ...
                {
                    column--;
                    if (game.Fields[column, row].Owner == 0 || game.Fields[column, row].Owner == 1)
                        return false;
        ...
        {
            if (game.Fields[oldXCoor, oldYCoor].Owner == 1)
                return true;
            return false;
        }
        ...
        {
            if (game.Fields[XCoor, YCoor].Owner == 0)
                return true;
            return false;
        }
